...
//   findeOne({ where }) {
//     const restaurant = this.remainTable.filter((el) => {
//       return el.restaurant._id === where.id;
//     });
//     if (restaurant.length > 0) return restaurant[0];
//     throw new ConflictException('식당정보가 없습니다.');
//   }

//   findeOneRemainTable({ where }) {
//     const restaurant = this.remainTable.filter((el) => {
//       console.log(el.remainTable, el.restaurant._id, where.id);
//       return el.remainTable === 0 && el.restaurant._id === where.id;
//     });

//     if (restaurant.length > 0) return restaurant[0];
//     return this.findeOne({ where });
//   }

//   delete({ restaurant_id, table }) {
//     const restaurant = this.findeOne({ where: { id: restaurant_id } });
//     if (restaurant.remainTable < 20) {
//       this.remainTable.forEach((el) => (el.remainTable += table));
//       return { data: true };
//     }
//   }

//   get({ restaurantId }) {
//     const isRemainTable = this.findeOneRemainTable({
//       where: { id: restaurantId },
//     });

//     if (isRemainTable.remainTable <= 0)
//       throw new UnprocessableEntityException(
//         '더이상 예약 가능한 테이블이 없습니다.',
//       );
//   }
// }
